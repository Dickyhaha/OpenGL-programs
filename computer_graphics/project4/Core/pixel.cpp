...
void Pixel::DrawLine(Vec2f Start, Vec2f End, Vec3f StartColor, Vec3f EndColor, bool NDC, bool Algorithm)
{	
	Vec2d StartPoint = NDC? (Start * (Size - 1)).round(): Vec2d(Start);
	Vec2d EndPoint = NDC? (End * (Size - 1)).round(): Vec2d(End);

	if(StartPoint < Vec2d() || EndPoint < Vec2d() || StartPoint >= Size || EndPoint >= Size)
		return;
	...
	if(dx == 0)
		Vertical(StartPoint, EndPoint, dx, dy, StartColor, EndColor);
	else if(dy == 0)
		Horizontal(StartPoint, EndPoint, dx, dy, StartColor, EndColor);
	else if(abs(dy) == abs(dx))
		Straight(StartPoint, EndPoint, dx, dy, StartColor, EndColor);
	else
	{
		Swap(StartPoint, EndPoint, StartColor, EndColor, dx, dy);

		if(Algorithm == ALGO_DDA)
			DDA(StartPoint, EndPoint, dx, dy, StartColor, EndColor);
		else
			Bresenham(StartPoint, EndPoint, dx, dy, StartColor, EndColor);
	}
}

//Bresenham's circle drawing algorihtm
void Pixel::DrawCircle(const Vec2f& Center, int Radius, const Vec3f& Color, bool NDC)
{
	int x = Radius;
	int y = 0;
	int err = 0;

	Vec2d Point = NDC? (Center * (Size - 1)).round(): Vec2d(Center);

	while(x >= y)
	{
		SetPixelColor(Vec2d(Point.x + x, Point.y + y), Color);
		SetPixelColor(Vec2d(Point.x + y, Point.y + x), Color);
		SetPixelColor(Vec2d(Point.x - y, Point.y + x), Color);
		SetPixelColor(Vec2d(Point.x - x, Point.y + y), Color);
		SetPixelColor(Vec2d(Point.x - x, Point.y - y), Color);
		SetPixelColor(Vec2d(Point.x - y, Point.y - x), Color);
...
void Pixel::DrawCurve(const std::vector<Vec2f>& Point, const Vec3f& Color)
{
	Vec2d Factor = Size - 1;
	for(unsigned int p = 0; p < Point.size() - 1; p++)
	{
		Vec2d v1 = (Point[p] * Factor).round();
		Vec2d v2 = (Point[p + 1] * Factor).round();
...
void Pixel::DrawNumber(const Vec2f& Position, int Value, const Vec3f& Color, bool NDC)
{
	char number[100];
	...
	Vec2d Point = NDC? Position * (Size - 1) - Vec2d(0, 20): Position;

	int offset = Point.x - num * 7 / 2;

	for(int n = 0; n < num; n++)
		for(int i = 0; i < 7; i++)
			for(int j = 0; j < 5; j++)
				if(Digit[number[n] - '0'][i][j])
					SetPixelColor(Vec2d(offset + n * 7 + j,  Point.y + (6 - i) - 3), Color);
}
